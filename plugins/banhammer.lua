...
local function kick_ban_res(extra, success, result)
      ...
      elseif get_cmd == 'unban' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] حذف بن شد')
      ...
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] حذف گلوبال بن شد')
...
local support_id = msg.from.id
 if matches[1]:lower() == 'id' and msg.to.type == "chat" or msg.to.type == "user" then
    ...
    elseif matches[1]:lower() == 'id' then
      ...
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "مشخصات گروه " ..string.gsub(msg.to.print_name, "_", " ").. ":\n\n"..msg.to.id
    end
  end
  ...
  if matches[1]:lower() == "banlist" then -- لیست بن ها
    local chat_id = msg.to.id
...
if matches[1]:lower() == 'kick' then
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'کاربر ['..user_id..' ] گلوبال بن شد'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'کاربر ['..user_id..' ] حذف گلوبال بن شد'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
  ...
  if matches[1]:lower() == "gbanlist" then -- لیست گلوبال بن ها
    return banall_list()
  end
end
